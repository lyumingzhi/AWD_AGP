...
- `inpainting/AWD_AGP/mask_RPN/`: Faster R-CNN based watermark-location proposal code; its training records are pseudo-labels generated by `official_surrogate_generate_AE_mask_DE_without_noise.py` using the superpixel-guided evolutionary search. See `docs/TRAIN_MASK_RPN.md` to generate records and train your own checkpoint.
