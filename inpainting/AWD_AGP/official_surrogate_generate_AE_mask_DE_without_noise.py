...
from inpainting.AWD_AGP.options import get_args
...
import time 
import random
from inpainting.AWD_AGP.official_utils import rbf_interpolation, ND_interp, generate_rect_mask
...
# from inpainting.AWD_AGP.official_utils import rbf_interpolation
from scipy.stats import rv_discrete


from inpainting.AWD_AGP.official_utils import rbf_interpolation, ND_interp,get_superpixel,get_semantic_loss

# evaluation func for evolution
def evaluation_for_evolution(input_parameter, img, rect, original_output, DE_records, best_validation_score, search_size, relative_mask_size, opt, original_FeatList, mask_size , mask_score_map, activated_mask, target_patch, attacker, superpixel_map):
    mask_index=[0,0]
    ...
    if 'semantic_attack' in opt.algorithm:
        # print('superpixel_map', superpixel_map.size(), 'mask', mask.size())
        # exit(0)
...
def main():
    ...
    all_models={'Crfillnet':None,'Gennet':None,'FcFnet':None,'Matnet':None, 'RFRnet':None}
    ...
    for target_model in opt.target_models:
        ...
        Path(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'result/')).mkdir(parents=True,exist_ok=True)
        Path(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'annot_result/')).mkdir(parents=True,exist_ok=True)
        Path(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'attacked_input/')).mkdir(parents=True,exist_ok=True)
        Path(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'attacked_mask/')).mkdir(parents=True,exist_ok=True)

        Path(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'validation_output/')).mkdir(parents=True,exist_ok=True)

        Path(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'DE_records/')).mkdir(parents=True,exist_ok=True)
        ...
        # for index,(img, mask,rect,target_patch) in enumerate(iter(dataloader)):
            ...
            if 'random_search' in opt.algorithm or True:
                ...
                args_for_DE_obj = (img, rect, original_output, DE_records, best_validation_score, search_size, relative_mask_size, opt, {}, mask_size, mask_score_map, activated_mask, None, attacker, superpixel_map)
            ...
            """ for i in range(attacked_output[list(attacked_output.keys())[0]].size()[0]):
                ...
                for name in attacked_output.keys():
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'result/attacked_'+name+'_'+str(preprocess_num)+'.png'),attacked_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'result/diff_'+name+'_'+str(preprocess_num)+'.png'),torch.abs(original_output[name]-attacked_output[name])[i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'result/output_with_mask_'+name+'_'+str(preprocess_num)+'.png'),((transforms.Resize((attacked_output[name  ][i].size()[-2:]))(output_mask)[i]>0.9)*attacked_output[name][i]).detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'annot_result/attacked_'+name+'_'+str(preprocess_num)+'.png'),attacked_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                ...
                assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'result/original_'+'_'+str(preprocess_num)+'.png'),original_output[list(original_output.keys())[0]][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])

                if output_mask!=None:
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'attacked_mask/mask_'+'_'+str(preprocess_num)+'.png'),(output_mask[i]>0.9).detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                ...
                if 'semantic_attack' in opt.algorithm:
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'annot_result/superpixel_map_'+str(preprocess_num)+'.png'),superpixel_map_vis.transpose(1,2,0)*255)
                
                assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'result/mask_score_map_'+name+'_'+str(preprocess_num)+'.png'),inter_result_to_show*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                preprocess_num+=1
                print('preprocess num',preprocess_num) """
            
            # save DE records including the intermediate results
            with open(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'DE_records', filename+'.json'),'w') as f:
                json.dump(DE_records,f,indent=4)
                print('filename',filename)
                print('DE_records',os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'DE_records', filename+'.json'))
         
        
        """ print('average_time_cost',average_time_cost/num_of_attack)
        rect_list['average_time_cost']=average_time_cost/num_of_attack
        with open(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'rect.json'),'w') as f:
