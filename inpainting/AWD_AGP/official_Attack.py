...
# from inpainting.AWD_AGP.clustering_measure import ADMM_attacker
...
# attacker class
class Attacker():  
    def __init__(self,opt,target_models,validation_model={},lr=0.01,iters=150,budget=0.03):
        self.opt=opt
    ...
    def calc_Content_Loss(self,features, targets, weights=None):
        ...
        for name in features.keys():
            ...
            for f, t, w in zip(features[name], targets[name], weights):
                single_content_loss = self.mse_criterion(f, t) * w
                if name in [ 'WDModel', 'DBWEModel', 'SLBRModel']:
                    tmp_content_loss -= single_content_loss
                else:
                    tmp_content_loss += single_content_loss
                    # print('inpaint content loss', single_content_loss)
                # print(name, 'has content loss:', single_content_loss.item())
        ...
        for name in features.keys():
            for f, t, w in zip(features[name], targets[name], weights):
                if name in [ 'WDModel', 'DBWEModel', 'SLBRModel']:
                    ...
                    gram_loss += self.mse_criterion(self.gram(f), self.gram(t)) * w
                    # print('inpaint loss gram loss', self.mse_criterion(self.gram(f), self.gram(t)) * w)
        return gram_loss
    ...
    def extract_features(self, model, x, layers,mask=None,rect=None):
        ...
        for name in x.keys():
            # print('rect', rect)
            # print('original output', x[name].shape, 'name', name)
            features = list()
            ...
            if rect != None:
                ...
                assert cv2.imwrite('inpainting/AWD_AGP/intermediate_output/extracted_region.jpg',single_x[0].detach().cpu().numpy().transpose(1,2,0)*255)

            else:
                single_x=x[name]
                # print(name, ' ', single_x.shape)
                assert cv2.imwrite('inpainting/AWD_AGP/intermediate_output/input_images/extracted_region.jpg',single_x[0].detach().cpu().numpy().transpose(1,2,0)*255)
            if not isinstance(model,dict):
                for index, layer in enumerate(model):
                    # print('single_x,', single_x.shape)
    ...
    """ def ADMM(self,input,noise_func,mask,target_patch,original_FeatList=None):
        ...
        for i in tqdm(range(self.opt.max_search_times),position=1):
            ...
            for mm in range(1,self.opt.maxIter_mm+1):
                ...
                assert cv2.imwrite('inpainting/AWD_AGP/experiment_result/'+str(mm)+'.jpg',mask.detach().cpu()[0].numpy().transpose(1,2,0)*255)
            mask=(mask>0.5).float()
    ...
    def attack(self,input,original_output,original_mask,target_patch=None,rect=None,original_FeatList=None, index = None):
        ...
        input_from_begining = input.cuda().detach()
        ...
        if 'attribution_attack_for_multi_task' in self.opt.algorithm:
            ...
            # assert cv2.imwrite('inpainting/AWD_AGP/IG_for_inpainting.png', (IG_for_inpainting_after_norm)[0].detach().cpu().numpy().transpose(1,2,0)*255)
            # assert cv2.imwrite('inpainting/AWD_AGP/IG_for_watermark_remover.png', (IG_for_watermark_remover_after_norm)[0].detach().cpu().numpy().transpose(1,2,0)*255)
        ...
        with torch.enable_grad():
            ...
            for i in range(self.iters):
                ...
                for tmp_input in preprocessed_inputs_with_noise:
                    ...
                    # untarget attack (common case)
                        ...
                        # Attribute-weight perturbation attack: extract attributes on adversaral examples in each iterations as weight and combine the perturbation from different models with different weights
                        if 'attribution_attack_for_multi_task3' in self.opt.algorithm:
                            ...
                            # assert cv2.imwrite('inpainting/AWD_AGP/pure_IG_for_watermark_remover_st_AE_before_norm.png', (torch.abs(IG_for_watermark_remover_AE)/torch.max(torch.abs(IG_for_watermark_remover_AE)))[0].detach().cpu().numpy().transpose(1,2,0)*255)
                            # assert cv2.imwrite('inpainting/AWD_AGP/pure_IG_for_inpainting_st_AE_before_norm.png', (torch.abs(IG_for_inpainting_AE)/torch.max(torch.abs(IG_for_inpainting_AE)))[0].detach().cpu().numpy().transpose(1,2,0)*255)
                            ...
                            # assert cv2.imwrite('inpainting/AWD_AGP/intermediate_output/pure_IG_for_all_task_st_AE_before_norm'+str(i)+'.png', (torch.abs(IG_for_inpainting_AE_after_norm + IG_for_watermark_remover_AE_after_norm)/torch.max(torch.abs(IG_for_inpainting_AE_after_norm + IG_for_watermark_remover_AE_after_norm)))[0].detach().cpu().numpy().transpose(1,2,0)*255)
                            ...
                            # assert cv2.imwrite('inpainting/AWD_AGP/intermediate_output/attacked_result_of_AE_for_wrm'+str(i)+'.jpg', self.target_models['SLBRModel'](AE_for_wrm)[0][0].detach().cpu().numpy().transpose(1,2,0)*255)
                    ...
                    # separate RIG attack 
                    # AGP (the one proposed in the paper): extract attributes on adversaral examples in each iterations and apply weighted RIG attack.
                    if 'TIG_sep' in self.opt.lossType:
                        if self.opt.target_attack:
                            IG,tmp_loss=self.integrated_gradient(tmp_input,mask,target_output,steps=10)
                        else: # default steps = 30
                            IG,tmp_loss=self.integrated_gradient(tmp_input,mask,original_output,steps=1, mode = 'separate', index_img = index, index_step=i, original_input= original_input)
                ...
                if 'norm_combine' in self.opt.algorithm and inter_grad!=None and 'TIG' not in self.opt.lossType and 'TIG_sep' not in self.opt.lossType and 'EG' not in self.opt.lossType and 'attribution_attack_for_multi_task' not in self.opt.algorithm and 'attribution_attack_for_multi_task2' not in self.opt.algorithm  and 'attribution_attack_for_multi_task3' not in self.opt.algorithm:
                    ...
                    if torch.is_tensor(sum_grad_for_inpainting):
                        with open(os.path.join('inpainting/AWD_AGP/sep_perturbations/baseline_grad_sep', 'inpainting'+str(index)+'_'+str(i)+'.npy'), 'wb') as  f:
                            perturbation_for_inpainting = (sum_grad_for_inpainting.data/torch.norm(sum_grad_for_inpainting.data, p=1)).detach().cpu().numpy()

                            assert cv2.imwrite('inpainting/AWD_AGP/inpainting_image.jpg', np.sign(np.abs(perturbation_for_inpainting)/np.max(np.abs(perturbation_for_inpainting)))[0].transpose(1,2,0)*255)
                            ...
                            assert cv2.imwrite('inpainting/AWD_AGP/inpainting_filtered_image.jpg', np.sign(np.abs(perturbation_for_inpainting)/np.max(np.abs(perturbation_for_inpainting)))[0].transpose(1,2,0)*255)
                            
                    
                    if torch.is_tensor(sum_grad_for_watermark_removal):
                        with open(os.path.join('inpainting/AWD_AGP/sep_perturbations/baseline_grad_sep', 'bwr'+str(index)+'_'+str(i)+'.npy'), 'wb') as  f:
            ...
            print('in attack', self.budget)
            # noise_func.delta_x.data = torch.clamp((torch.clamp(noise_func.delta_x.detach()+input,min=0,max=1) - input), min=-self.budget, max=self.budget).detach()
            noise_func.delta_x.data = torch.clamp((torch.clamp(noise_func.delta_x.detach() + input_from_begining, min=0, max=1) - input_from_begining), min=-self.budget, max=self.budget).detach()
            print('delta x', noise_func.delta_x.data.min(), noise_func.delta_x.data.max())
            # input_with_final_noise = noise_func(input.detach().clone())
            input_with_final_noise = noise_func(input_from_begining.detach().clone())

            print('input_with_final_noise', (input_with_final_noise[list(self.target_models.keys())[0]] - input_from_begining).min(), (input_with_final_noise[list(self.target_models.keys())[0]] - input_from_begining).max(),)
        ...
        return input_with_final_noise[list(self.target_models.keys())[0]], (input_with_final_noise[list(self.target_models.keys())[0]]-input_from_begining).detach().cpu(), evaluate_output, mask.detach().cpu(), validation_output, original_output_for_validation,validation_value, input_from_begining
    
        # return input_with_final_noise[list(self.target_models.keys())[0]], (input_with_final_noise[list(self.target_models.keys())[0]]-original_input).detach().cpu(), evaluate_output, mask.detach().cpu(), validation_output, original_output_for_validation,validation_value, input_from_begining
    ...
    def expectation_gradient(self,input,mask,y,steps,tau=0.1):
        ...
        for index in range(steps+1):
            ...
            # assert cv2.imwrite('inpainting/AWD_AGP/original_output.jpg',original_outputs[list(original_outputs.keys())[0]][0].detach().cpu().numpy().transpose(1,2,0)*255)
            # exit()
            if self.opt.target_attack:   
            ...
            # for name in self.target_models:
            #     Path('inpainting/AWD_AGP/IG_attribute_'+name).mkdir(parents=True,exist_ok=True)
    
            #     assert cv2.imwrite('inpainting/AWD_AGP/IG_attribute_'+name+'/IG_final_output_'+name+str(index)+'.jpg',tmp_outputs[name][0].detach().cpu().numpy().transpose(1,2,0)*255)
        ...
        # assert cv2.imwrite('inpainting/AWD_AGP/input_example.jpg',input[list(input.keys())[0]][0].detach().cpu().numpy().transpose(1,2,0)*255)
        assert cv2.imwrite('inpainting/AWD_AGP/EG.jpg',(np.abs(EG)/np.max(np.abs(EG)))[0].transpose(1,2,0)*255)
        # exit()
    ...
    def integrated_gradient(self,input,mask,y,steps,tau=0.1, mode = 'all', baseline = None, index_img = None, index_step = None, original_input = None ):
        ...
        with torch.no_grad():
            name = list(input.keys())[0]
            baseline = input[name].clone().detach() 
        ############
        ...
        for index in range(steps+1):
            ...
            scaled_input=torch.clamp((baseline + ((index/steps)*(input[name]-baseline)) ).detach(), min =0, max =1)
            scaled_input.requires_grad=True
            ...
            def print_grad(name, mode = 'input'):
                if inter_grad is None:
                    ...
                    def grad_func(grad):
                        ...
                        # print(name, 'in inter gard', grad.sum().item())
                        inter_grad[name]=grad
                    return grad_func
            ...
            for name in self.target_models.keys():                
                ...
                if name in self.watermark_remover_names:
                    ...
                    tmp_output=self.target_models[name](cloned_scaled_inputs[name], mask)
                    # print('cloned_scaled_inputs[name]', cloned_scaled_inputs[name].shape, name)
                    # print('tmp_output in ', name, tmp_output[0].shape, tmp_output[1].shape, tmp_output[2].shape)
            ...
            if mode != 'separate':
                ...
                for name in self.target_models:
                    ...
                    if torch.sum(torch.isnan(inter_grad[name])):
                        ...
                        assert cv2.imwrite('inpainting/AWD_AGP/problem_output.jpg', tmp_outputs[name].detach().cpu()[0].numpy()[::-1].transpose(1,2,0) * 255)
            ...
            # print('grads', [(grads[i]['inpainting']['FcFnet'].sum(), grads[i]['wr']['WDModel'].sum()) for i in range(len(grads))])
        ...
        # combine IG by separately normalize the IGs from different types of watermark removers separately
            ...
            if mean_grad_for_inpainting != {}:
                ...
                assert cv2.imwrite('inpainting/AWD_AGP/inpainting_grey_numpy.jpg', np.sign(np.abs(inpainting_grey_numpy))[0].transpose(1,2,0)*255)
                with open(os.path.join('inpainting/AWD_AGP/sep_perturbations/', 'ours_TIG_sep','inpainting'+str(index_img)+'_'+str(index_step)+'.npy'), 'wb') as f:
            ...
            for name in self.target_models:
                ...
                # print('mean_grad_for_wr', mean_grad_for_wr[name], name)
            ...
            if mean_grad_for_wr!={}:
                ...
                assert cv2.imwrite('inpainting/AWD_AGP/bwr_grey_numpy.jpg', np.sign(np.abs(bwr_grey_numpy))[0].transpose(1,2,0)*255)
                with open(os.path.join('inpainting/AWD_AGP/sep_perturbations/', 'ours_TIG_sep','bwr'+str(index_img)+'_'+str(index_step)+'.npy'), 'wb') as f:
            ...
            # assert cv2.imwrite('inpainting/AWD_AGP/RIG_figure_for_paper/mean_grad_for_wr_for_paper'+str(figure_for_paper_index)+'.jpg', avg_grads_for_wr_grey)
            ...
            delta_X=input[list(input.keys())[0]].detach() - baseline
            ############
            delta_X = torch.ones_like(delta_X).cuda()
            ###########
            integrated_grad_for_inpainting = avg_grads_for_inpainting * delta_X
            integrated_grad_for_wr = avg_grads_for_wr * delta_X
            print('wr grad', torch.sum(integrated_grad_for_wr), torch.norm(integrated_grad_for_wr, p=1), 'inpainting', torch.sum(integrated_grad_for_inpainting), torch.norm(integrated_grad_for_inpainting, p=1))

            if torch.norm(integrated_grad_for_inpainting, p=1).item() == 0:
                print('integrated_grad_for_inpainting IG is 0', )
                IG = (integrated_grad_for_wr/torch.norm(integrated_grad_for_wr, p=1)).detach().cpu().numpy()
            elif torch.norm(integrated_grad_for_wr, p=1).item() == 0:
                IG = (integrated_grad_for_inpainting/torch.norm(integrated_grad_for_inpainting, p=1)).detach().cpu().numpy()
                print('integrated_grad_for_inpainting IG is 0')
            else:
        ...
        # assert cv2.imwrite('inpainting/AWD_AGP/input_example.jpg',input[list(input.keys())[0]][0].detach().cpu().numpy().transpose(1,2,0)*255)
        # assert cv2.imwrite('inpainting/AWD_AGP/IG_'+mode+'.jpg',np.abs(np.abs(IG)/np.max(np.abs(IG)))[0].transpose(1,2,0)*255)
        # exit()
    ...
    def simple_attack(self,input,mask,y,steps,tmp_target_models = None, iter_index = None, clean_input = None, rect = None):
        ...
        if 'SLBRModel' in tmp_target_models:
            assert cv2.imwrite('inpainting/AWD_AGP/intermediate_output/wr'+str(iter_index)+'.jpg', tmp_target_models['SLBRModel'](noise_func.delta_x.data + clean_input)[0][0].detach().cpu().numpy().transpose(1,2,0)*255)
            del tmp_outputs
        ...
        if 'SLBRModel' in tmp_target_models:
            assert cv2.imwrite('inpainting/AWD_AGP/intermediate_output/wr_after_final_norm'+str(iter_index)+'.jpg', tmp_target_models['SLBRModel'](input_with_final_noise['SLBRModel'])[0][0].detach().cpu().numpy().transpose(1,2,0)*255)
