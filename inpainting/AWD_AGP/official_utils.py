...
from inpainting.AWD_AGP.superpixel_fcn import models 
...
def rbf_interpolation(score,map):
    ...
    cv2.imwrite('inpainting/AWD_AGP/score_map.jpg',(torch.abs(score)/torch.max(torch.abs(score))).detach().cpu().numpy()*255)
    ...
    cv2.imwrite('inpainting/AWD_AGP/interpolated_score_map.jpg',(torch.abs(mean_t)/torch.max(torch.abs(mean_t))).detach().cpu().numpy()*255)
    return mean_t.view(map.size()[-2],map.size()[-1])
    cv2.imwrite('inpainting/AWD_AGP/meant.jpg',mean_t.detach().cpu().numpy()*255)
...
def ND_interp(score,map):
    ...
    # cv2.imwrite('inpainting/AWD_AGP/score_map_SBS.jpg',((score1-np.min(score1))/(np.max(score1)-np.min(score1)))*255)
    ...
    # cv2.imwrite('inpainting/AWD_AGP/interpolated_score_map_Linear.jpg',inter_result_to_show*255)
    # exit()
...
def get_semantic_loss(superpixel_map,mask,w1=100,w2=80,w3=60):
    ...
    # assert cv2.imwrite('inpainting/AWD_AGP/superpixel_shift_img.jpg', (shift>0)[0].detach().cpu().numpy()*255)
...
def generate_rect_mask_for_dataset(im_size, mask_size, margin=8, rand_mask=False, position = None):
    ...
    if position == 'random':
        ...
        if position == 'center' or (position is None):
            ...
            print( (mask_size[0], im_size[0] - 1 - position[0]),  (mask_size[1], im_size[1] - 1 - position[2]))
...
def preprocess_img_for_attackDemo(img, _bbox, mask_size, index_of_logo = 0, logo=None):
    ...
    # obtain the mask image and formatted rectangular coordinates
    if _bbox is not None:
        mask, rect = generate_rect_mask_for_dataset((int(img.shape[-2] ),int(img.shape[-1] )), (int(mask_size[0]), int(mask_size[1])), rand_mask=False, position = np.array([int(_bbox[0][0, 0]), 0, int(_bbox[0][0, 1]), 0]))
    else:
        mask, rect = generate_rect_mask_for_dataset((int(img.shape[-2] ),int(img.shape[-1] )), (int(mask_size[0]), int(mask_size[1])), rand_mask=False, position = 'random')
    ...
    if logo is None:
        logo = cv2.imread(os.path.join('inpainting/logo/train_logo', str(index_of_logo%38 +1)+'.png'), cv2.IMREAD_UNCHANGED)

    print('log size', logo.shape)
    if logo.shape[2] == 4:
        logo_alpha = (np.sum((logo[..., 1:]), axis=-1) > 0)[..., np.newaxis].astype(np.uint8) * 255
        # logo_alpha = logo[..., 0]
    else:
        logo_alpha = np.ones((*logo.shape[:-1], 1))
    print(logo[0,0,:], logo_alpha[0,0,:])
    logo = logo[..., 1:]

    logo = mask_transformer(logo)
    logo_alpha = mask_transformer(logo_alpha)
    print('rect', rect)
...
if __name__=='!__main__':
    ...
    plt.savefig('inpainting/AWD_AGP/pykeop.png')
...
if __name__=='__main__':
    ...
    from inpainting.AWD_AGP.dataset import AttackDataset,TransferAttackDataset
    ...
    for index,(img, mask,rect,target_patch, logo) in enumerate(iter(dataloader)):
        ...
        # mask=cv2.imread('inpainting/AWD_AGP/experiment_result/pred_masks_from_latest_fasterrcnn_mask_instance_mixture/373.png')
        ...
        assert cv2.imwrite('inpainting/AWD_AGP/superpixel_maps/superpixel_graph_example'+str(index)+'.png', vis_graph.transpose(1,2,0)*255)
        # exit()
