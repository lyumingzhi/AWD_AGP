...
def main():
    ...
    for target_model in opt.target_models:
        ...
        Path(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'original_input/')).mkdir(parents=True,exist_ok=True)
            ...
            with torch.no_grad():
                ...
                for i in range(attacked_output[list(attacked_output.keys())[0]].size()[0]):
                    ...
                    for name in attacked_output.keys():
                        ...
                        assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'result/original_'+name+'_'+str(preprocess_num)+'.png'),original_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                        ...
                        assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'annot_result/original_'+name+'_'+str(preprocess_num)+'.png'),original_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])

                        # if the model is blind watermark remover, also save the detected mask of watermark
                        if name in [ 'WDModel', 'DBWEModel', 'SLBRModel']:
                            assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'annot_result/original_pred_mask_'+name+'_'+str(preprocess_num)+'.png'),original_pred_mask[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                            assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'annot_result/attacked_pred_mask_'+name+'_'+str(preprocess_num)+'.png'),attacked_pred_mask[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'attacked_input/',str(preprocess_num)+'.png'),input_with_final_noise[i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    
                    # save original input
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,target_model,'original_input/',str(preprocess_num)+'.png'),img[i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    preprocess_num+=1
