...
def main():
    ...
    # exit()
    # dataset=AttackDataset(opt,opt.InputImg_dir)
    ...
    for name_to_test in modelsToTest:
        ...
        dataset=TransferAttackDataset(opt,opt.InputImg_dir,opt.experiment_dir,'Matnet', if_sort=True)
        if dataset==None:
            continue
        ...
        for index,(img, adv_img, mask,rect,target_patch, logo) in enumerate(iter(dataloader)):
            ...
            attacked_mask_output = {}

            final_original_output = {}
            final_attacked_output = {}
            with torch.no_grad():
                ...
                # assert cv2.imwrite('inpainting/AWD_AGP/'+name_to_test+'_original_place2.jpg',(original_output[name_to_test])[0].detach().cpu().numpy().transpose(1,2,0)*255)
                # assert cv2.imwrite('inpainting/AWD_AGP/'+name_to_test+'_attacked_place2.jpg',(attacked_output[name_to_test])[0].detach().cpu().numpy().transpose(1,2,0)*255)
                # exit()

            original_mask_output = {key: value[1] for key, value in original_output.items()}
            attacked_mask_output = {key: value[1] for key, value in attacked_output.items()}

            final_original_output = {key: value[2] for key, value in original_output.items()}
            final_attacked_output = {key: value[2] for key, value in attacked_output.items()}

            original_output = {key: value[0] for key, value in original_output.items()}
            attacked_output = {key: value[0] for key, value in attacked_output.items()}
            ...
            #     # target_img[:,:,rect[0,0,0]:rect[0,0,0]+rect[0,0,1],rect[0,0,2]:rect[0,0,2]+rect[0,0,3]]=target_patch
            #     # assert cv2.imwrite('inpainting/AWD_AGP/target_img.jpg',target_img[0,...].detach().cpu().numpy().transpose(1,2,0)*255)
            #     # exit()
            ...
            #         input_with_final_noise,adv_noise,attacked_output=attacker.attack(img,original_output,mask,target_patch=target_patch,rect=rect,original_FeatList=original_FeatList)
            # else:    
            #     input_with_final_noise,adv_noise,attacked_output=attacker.attack(img,original_output,mask,original_FeatList=original_FeatList)
            ...
            for i in range(attacked_output[list(attacked_output.keys())[0]].size()[0]):
                ...
                for name in attacked_output.keys():
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name_to_test,'result/attacked_pred_mask_'+name+'_'+str(preprocess_num)+'.png'),attacked_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/', opt.output_dir,name_to_test,'result/original_pred_mask_'+name+'_'+str(preprocess_num)+'.png'),original_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])

                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name_to_test,'result/final_attacked_'+name+'_'+str(preprocess_num)+'.png'),final_attacked_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/', opt.output_dir,name_to_test,'result/final_original_'+name+'_'+str(preprocess_num)+'.png'),final_original_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # exit()
                    # original_output[name][i,:,rect[i,0,0],rect[i,0,2]:rect[i,0,2]+rect[i,0,3]]=1
                    # original_output[name][i,:,rect[i,0,0]+rect[i,0,1],rect[i,0,2]:rect[i,0,2]+rect[i,0,3]]=1
                    # original_output[name][i,:,rect[i,0,0]:rect[i,0,0]+rect[i,0,1],rect[i,0,2]]=1
                    # original_output[name][i,:,rect[i,0,0]:rect[i,0,0]+rect[i,0,1],rect[i,0,2]+rect[i,0,3]]=1

                    # attacked_output[name][i,:,rect[i,0,0],rect[i,0,2]:rect[i,0,2]+rect[i,0,3]]=1
                    # attacked_output[name][i,:,rect[i,0,0]+rect[i,0,1],rect[i,0,2]:rect[i,0,2]+rect[i,0,3]]=1
                    # attacked_output[name][i,:,rect[i,0,0]:rect[i,0,0]+rect[i,0,1],rect[i,0,2]]=1
                    # attacked_output[name][i,:,rect[i,0,0]:rect[i,0,0]+rect[i,0,1],rect[i,0,2]+rect[i,0,3]]=1

                    # print('mask size',mask.size(),'attack output size',attacked_output[name].size())
                    # edge=transforms.Resize((attacked_output[name].size()[-1],attacked_output[name].size()[-1]))(edge)
                    # assert cv2.imwrite('inpainting/AWD_AGP/edge_example.jpg',edge[0].cpu().numpy().transpose(1,2,0)*255)
                    # exit()
                    if mask!=None:
                        ...
                        attacked_mask_output[name]=torch.clamp(attacked_mask_output[name][i]+transforms.Resize((attacked_mask_output[name].size()[-1],attacked_mask_output[name].size()[-1]))(edge),min=0,max=1)
                        final_attacked_output[name]=torch.clamp(final_attacked_output[name][i]+transforms.Resize((final_attacked_output[name].size()[-1],final_attacked_output[name].size()[-1]))(edge),min=0,max=1)

                        # original_output[name]=torch.clamp(original_output[name][i]+transforms.Resize((original_output[name].size()[-1],original_output[name].size()[-1]))(edge),min=0,max=1)
                        pass
                    ...
                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name_to_test,'result/attacked_mask_'+name+'_'+str(preprocess_num)+'.png'),attacked_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/', opt.output_dir,name_to_test,'result/original_mask_'+name+'_'+str(preprocess_num)+'.png'),original_mask_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])

                    assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name_to_test,'result/final_attacked_'+name+'_'+str(preprocess_num)+'.png'),final_attacked_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # assert cv2.imwrite(os.path.join('inpainting/AWD_AGP/experiment_result/', opt.output_dir,name_to_test,'result/final_original_'+name+'_'+str(preprocess_num)+'.png'),final_original_output[name][i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    # assert cv2.imwrite('inpainting/AWD_AGP/experiment_result/'+opt.output_dir+'/attacked_input/'+name+'_'+str(preprocess_num)+'.png',input_with_final_noise[i].detach().cpu().numpy().transpose(1,2,0)*255,[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    if rect.dim() == 3:
