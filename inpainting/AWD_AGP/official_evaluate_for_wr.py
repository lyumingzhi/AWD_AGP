...
import numpy as np
import os
from inpainting.AWD_AGP.official_dataset import AttackDataset, TransferAttackDataset
...
def get_img_edge(img):
    ...
    # assert cv2.imwrite('inpainting/AWD_AGP/edge_example.jpg', edges[0].detach().cpu().numpy().transpose(1,2,0)*255)
    # # print(torch.max(img))
    # assert cv2.imwrite('inpainting/AWD_AGP/edge_input_example.jpg', img[0].detach().cpu().numpy().transpose(1,2,0)*255)
    return edges
...
def save_detail_records(**records):
    ...
    # print(detail_df)
    writer = pd.ExcelWriter(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'detail_'+name+'_maskded_only_result_with_input_wo_mask.xlsx'), engine='xlsxwriter')

    f=open(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'detail_'+name+'_maskded_only_result_with_input_wo_mask.json'),'w')
    json.dump(detail_df,f,indent=4)
    # with open(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,name+'_result.json'),'w') as f:
...
def main():
    ...
    with open('inpainting/AWD_AGP/experiment_result/'+opt.output_dir+'/rect.json') as f:
        rect=json.load(f)
    ...
    print(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.source_dir,'Gennet','attacked_mask/'))
    
    mask_index=0
    for root,dirs,files in os.walk(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.source_dir,'Matnet','attacked_mask/')):
        for file in files:
    ...
    if 'random_mask' in opt.algorithm:
        mask_index=0
        for root,dirs,files in os.walk(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'random_mask/')):
            for file in files:
    ...
    for name in opt.target_models:
        ...
        #     continue
        name_to_test = name
        opt.experiment_dir = os.path.join('inpainting/AWD_AGP/experiment_result', opt.source_dir)
        ...
        for i,mask_path in enumerate(mask_paths):
            ...
            # attacked_img=cv2.imread('inpainting/AWD_AGP/experiment_result/'+opt.output_dir+'/result/attacked_'+name+'_'+str(i)+'.png')
            # original_img=cv2.imread('inpainting/AWD_AGP/experiment_result/'+opt.output_dir+'/result/original_'+name+'_'+str(i)+'.png')

            print('current file', os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/attacked_'+name+'_'+str(i)+'.png'))
            attacked_img=cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/attacked_'+name+'_'+str(i)+'.png')).astype(float)/255
            
            original_img=cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/original_'+name+'_'+str(i)+'.png')).astype(float)/255

            # original_mask = cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/original_pred_mask_'+name+'_'+str(i)+'.png')).astype(float)/255

            # attacked_mask = cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/attacked_pred_mask_'+name+'_'+str(i)+'.png')).astype(float)/255

            original_pred_logo_mask = cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/original_pred_mask_'+name+'_'+str(i)+'.png'))
            original_pred_logo_mask = cv2.cvtColor(original_pred_logo_mask, cv2.COLOR_BGR2GRAY).astype(float)/255
            attacked_pred_logo_mask = cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/attacked_pred_mask_'+name+'_'+str(i)+'.png'))
            attacked_pred_logo_mask = cv2.cvtColor(attacked_pred_logo_mask, cv2.COLOR_BGR2GRAY).astype(float)/255

            original_final_img = cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/final_original_'+name+'_'+str(i)+'.png')).astype(float)/255
            attacked_final_img = cv2.imread(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,'result/final_attacked_'+name+'_'+str(i)+'.png')).astype(float)/255
            ...
            # assert cv2.imwrite('inpainting/AWD_AGP/original_input_in_eval.jpg', original_input)
            # assert cv2.imwrite('inpainting/AWD_AGP/attacked_img_in_eval.jpg', attacked_img)
            # assert cv2.imwrite('inpainting/AWD_AGP/original_img_in_eval.jpg', original_img)
            # print`(rect.keys())
            # if i == 2:
            #     exit()  
            ...
            # assert cv2.imwrite('inpainting/AWD_AGP/original_patch.jpg', original_patch.detach().cpu().numpy().transpose(1,2,0)*255)
            ...
            # assert cv2.imwrite('inpainting/AWD_AGP/masked_original_inputs.jpg', original_input*mask*255)
            ...
            # assert cv2.imwrite('inpainting/AWD_AGP/original_inputs_patch.jpg', original_inputs_patch.detach().cpu().numpy().transpose(1,2,0)*255)
            ...
            if opt.target_attack:
                ...
                if 'random_mask' in opt.algorithm:
                    ...
                    # assert cv2.imwrite('inpainting/AWD_AGP/example_attach_patch.jpg', attacked_patch.detach().cpu().numpy().transpose(1,2,0) * 255)
                    # assert cv2.imwrite('inpainting/AWD_AGP/example_original_patch.jpg', original_patch.detach().cpu().numpy().transpose(1,2,0) * 255)
                    # assert cv2.imwrite('inpainting/AWD_AGP/example_original_inputs_patch.jpg', original_inputs_patch.detach().cpu().numpy().transpose(1,2,0) * 255)
                    # assert cv2.imwrite('inpainting/AWD_AGP/example_original_inputs_wo_logo_patch.jpg', original_inputs_wo_logo_patch.detach().cpu().numpy().transpose(1,2,0) * 255)
                    # assert cv2.imwrite('inpainting/AWD_AGP/example_attacked_final_patch.jpg', attacked_final_patch.detach().cpu().numpy().transpose(1,2,0) * 255)
                    # assert cv2.imwrite('inpainting/AWD_AGP/example_original_final_patch.jpg', original_final_patch.detach().cpu().numpy().transpose(1,2,0) * 255)
                    # exit()
            ...
            if opt.target_attack:
                ...
                if 'random_mask' in opt.algorithm:
                    ...
                    # cv2.imwrite('inpainting/AWD_AGP/attacked_patch_in_evaluate_wr.jpg', attacked_patch.detach().cpu().numpy().transpose(1,2,0)*255)
                    # cv2.imwrite('inpainting/AWD_AGP/original_input_patch_in_evaluate_wr.jpg', original_inputs_patch.detach().cpu().numpy().transpose(1,2,0)*255)
                    # exit()
    ...
    writer = pd.ExcelWriter(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,name+'_maskded_only_result_for_wr.xlsx'), engine='xlsxwriter')
    ...
    f=open(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,name+'_maskded_only_result_for_wr.json'),'w')
    json.dump(df,f,indent=4)
    # with open(os.path.join('inpainting/AWD_AGP/experiment_result/',opt.output_dir,name,name+'_result.json'),'w') as f:
    #     json.dump(df,f,indent=4)
    df=pd.DataFrame(df)
