...
class AttackDataset(torchvision.datasets.ImageFolder):
    def __init__(self,opt,img_dir,relative_mask_size=0.33,if_sort=False, transform=None, target_transform=None):
        ...
        self.watermark_alpha_list = None
        alpha_file = 'inpainting/AWD_AGP/random_logo_alpha.json'
        if 'random_logo_alpha' in self.opt.algorithm and os.path.exists(alpha_file):
    ...
    def __getitem__(self,index):
        ...
        # load mask from the predefined mask directory
            ...
            if self.rect_file is not None:
                ...
                mask_path = os.path.join(self.opt.RPNRefineMask, 'attacked_mask', 'mask__' + str(index) + '.png')
                if os.path.exists(mask_path):
                    mask = cv2.imread(mask_path)
                    mask = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)
                    mask = np.expand_dims(mask.astype(float) / 255 > 0.5, axis=0)
                else:
                    mask, _ = self.generate_rect_mask(
                        (int(img.size()[-2]), int(img.size()[-1])),
        ...
        if self.opt.get_logo == True:
            ...
            if self.opt.attach_logo == True:
                if self.watermark_alpha_list is not None:
                    watermark_alpha = self.watermark_alpha_list[index]
                else:
                    watermark_alpha = getattr(self.opt, 'watermark_alpha', 0.1)
